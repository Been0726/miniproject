...
class CommentRepositoryTest {
    ...
    void findByArticleId() {
        ...
        /* Case 3 : 9번 게시글의 모든 댓글 조회 */
        {
            ...
            List<Comment> expected = Arrays.asList();
            // 4. 비교 및 검증
            assertEquals(expected.toString(), comments.toString(), "9번 글은 없으므로 댓글 없음");
        }
        /* Case 4 : 999번 게시글의 모든 댓글 조회 */
        {
            ...
            List<Comment> expected = Arrays.asList();
            // 4. 비교 및 검증
            assertEquals(expected.toString(), comments.toString(), "999번 글은 없으므로 댓글 없음");
        }
        {
            // 1. 입력 데이터 준비
            Long articleId = -1L;
            ...
            List<Comment> expected = Arrays.asList();
            // 4. 비교 및 검증
            assertEquals(expected.toString(), comments.toString(), "-1번 글은 없으므로 댓글 없음");
        }
    }
    ...
    void findByNickname() {
        ...
        /* Case 2 : "Kim"의 모든 댓글 조회 */
        {
            ...
            List<Comment> comments = commentRepository.findByNickname(nickname);
            // 3. 예상 데이터
            Comment a = new Comment(2L, new Article(4L, "당신의 인생 영화는?", "댓글 고"), "Kim", "아이 엠 샘");
            Comment b = new Comment(5L, new Article(5L, "당신의 소울 푸드는?", "댓글 고고"), "Kim", "샤브샤브");
        ...
        /* Case 3 : null의 모든 댓글 조회 */
        {
            ...
            List<Comment> expected = Arrays.asList();
            // 4. 비교 및 검증
            assertEquals(expected.toString(), comments.toString(), "null은 댓글이 없음");
        }
        /* Case 4 : ""의 모든 댓글 조회 */
        {
            // 1. 입력 데이터 준비
            ...
            List<Comment> expected = Arrays.asList();
            // 4. 비교 및 검증
            assertEquals(expected.toString(), comments.toString(), "''은 댓글이 없음");
        }
    }
}